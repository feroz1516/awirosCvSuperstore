...
def postprocess_yolo(outputs,ori_image,dwdh,ratio,names,conf_thresh):
    print("postprocess_yolo is called...")
    blobs = []
    image = ori_image.copy()
    # conf_thresh = 0.0
    print("result of post process yolo", outputs)

    boxes = outputs[:,:4, :]
    confidences = outputs[:,4:, :]
    print("boxes of post process yolo", boxes)
    ...
    with open("detections.txt", "w") as detection_file:
                detection_file.write("dwdh: " + str(detections))
    predictions = []
    ...
    for i in range(len(detections)):    
        ...
        # print("x0: ",blb.tx, "x0 type: ",type(blb.tx))

        # print("x0: ",blb.ty, "x0 type: ",type(blb.ty))

        # print("x0: ",blb.bx, "x0 type: ",type(blb.bx))

        # print("x0: ",blb.by, "x0 type: ",type(blb.by))


        # print("Score: ", score, "Score type:", type(score))

        # print("cls id: ", cls_id, "cls id type: ",type(cls_id))
    
        predictions.append([int(blb.tx),int(blb.ty),int(blb.bx),int(blb.by),score])
        
        blb.id = random.randint(0,1000000)
        blb.label = name
        # print("blob tx: ",blb.tx)
        # print("blob ty: ",blb.ty)
        # print("blob bx: ",blb.bx)
        # print("blob by: ",blb.by)
    ...
    tracker.update(image, predictions)
    print("I am outside tracker")
    for track in tracker.tracks:
            print("I am inside tracker")
...
def post_process_yolov8(input_boxes, confidences, confidence_threshold=0.5, nms_threshold=0.4):
    ...
    print("Inside post_process_yolov8")
    ...
    # Initialize emp    ty lists to store the final detections
    detections = []
    ...
    print("I am stuck before return")
    return detections

# Perform Non-Maximum Suppression (NMS)
def nms(detections, threshold):
    print("NMS function is called")
    x1 = detections[:, 0]
    y1 = detections[:, 1]
    x2 = detections[:, 2]
    y2 = detections[:, 3]
    ...
    keep = []
    print("Entering while loop in nms...")
    while order.size > 0:
        i = order[0]
        keep.append(i)
    ...
    print("Exiting while loop in nms...")
